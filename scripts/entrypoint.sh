...
# --- FlashInfer JIT parallelism guard ---
# Without this, ninja defaults to ALL CPU cores (20 on GB10) for parallel nvcc
# compilation of CUTLASS MoE kernels. Each nvcc process uses ~8-16 GiB RAM, so
# 20 parallel jobs exhaust 121 GiB RAM and freeze the host.
# MAX_JOBS=6 limits to 6 parallel nvcc processes (~1.5 GiB peak total).
# FLASHINFER_NVCC_THREADS=2 passes --threads=2 to each nvcc invocation.
