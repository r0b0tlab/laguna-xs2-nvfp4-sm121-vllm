#!/usr/bin/env python3
"""NVFP4 KV cache benchmark with power telemetry on SM121.

Runs:
  1. Sanity suite (5 prompts: math, code, reasoning, factual, instruction-following)
  2. Prefill measurement (long single-stream prompts for TTFT/prefill rate)
  3. Concurrency ramp (c1/c2/c4/c8/c16/c32) with power telemetry
  4. KV cache / runtime facts from container logs

Usage:
  python3 benchmark_nvfp4_kv.py --base-url http://127.0.0.1:18080 --model Laguna-XS-2.1-NVFP4 --container sm121-laguna
"""
...
def main():
    ...
    ap.add_argument('--container', default='sm121-laguna')
    args = ap.parse_args()
    ...
    def chat(prompt, max_tokens=256):
        ...
        return {'seconds': dt, 'completion_tokens': obj['usage']['completion_tokens'],
                ...
                'ttft_proxy': dt - (obj['usage']['completion_tokens'] / max(dt, 0.001)) * 0.5}  # rough estimate
    ...
    # ─── Prefill Measurement ───
    print("\n" + "=" * 60)
    print("PREFILL MEASUREMENT (long single-stream prompts)")
    print("=" * 60)
    prefill = []
    # Use a single long prompt repeated to measure prefill throughput
    long_prompts = [
        ("short", "Explain quantum entanglement in detail. " * 5, 16),     # ~100 tok
        ("medium", "Explain quantum entanglement in detail. " * 30, 16),   # ~600 tok
        ("long", "Explain quantum entanglement in detail. " * 60, 16),     # ~1200 tok
        ("xlong", "Explain quantum entanglement in detail. " * 120, 16),   # ~2400 tok
    ]
    for name, prompt, max_tok in long_prompts:
        r = chat(prompt, max_tok)
        ptoks = r['prompt_tokens']
        prefill_rate = ptoks / r['seconds'] if r['seconds'] > 0 else 0
        print(f"  [{name:>6s}] {ptoks:>5d} prompt tok in {r['seconds']:.2f}s → {prefill_rate:>7.1f} tok/s prefill", flush=True)
        prefill.append({'name': name, 'prompt_tokens': ptoks, 'seconds': r['seconds'],
                        'prefill_rate_tok_s': prefill_rate, 'completion_tokens': r['completion_tokens']})
    ...
    logs = ""
    for cname in [args.container, 'sm121-laguna', 'sm121-vllm', 'sm121-vllm-kvexp']:
        try:
